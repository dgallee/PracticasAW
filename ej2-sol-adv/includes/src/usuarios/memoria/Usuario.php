...
class Usuario
{
    ...
    static function init()
    {
        ...
        if ($bd == null) {
            $bd = [
                'admin' => new Usuario('admin', password_hash('adminpass', PASSWORD_DEFAULT), 'Administrador', 1, [self::ADMIN_ROLE]),
                'user' => new Usuario('user', password_hash('userpass', PASSWORD_DEFAULT), 'Usuario', 2, [self::USER_ROLE])
            ];
